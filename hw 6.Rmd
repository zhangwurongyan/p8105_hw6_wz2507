...
boot_strap = data_frame(strap_number = 1:5000,
                    strap_sample = rerun(5000, boot_sample(weather_df)))

results = boot_strap %>% mutate(model = map(strap_sample, ~lm(tmax~tmin, data = .)),
                                summary = map(model, broom::glance),
                                coefficient = map(model, broom::tidy)) %>% 
  select(summary, coefficient, strap_number) %>% unnest() %>% mutate(term=recode(term, "(Intercept)" = "beta0", "tmin"="beta1")) %>%
  select(r.squared, adj.r.squared, term, estimate, strap_number)  %>%  pivot_wider(names_from = term, values_from = estimate) %>% mutate(log = log(beta0*beta1))

results[1:5,] %>% 
  select(strap_number, r.squared, adj.r.squared,log) %>% knitr::kable()
```
